...
# Discriminator Model - Modified for BCE Loss
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            # Removed nn.Sigmoid() for BCEWithLogitsLoss
        )
    
    def forward(self, x):
        out = self.net(x)
        return out.view(x.size(0), -1).mean(1)  # Average spatial scores per sample
...
def train(args):
    """Main training function with BCE Loss implementation."""
    # Setup device
    ...
    # Loss functions and optimizers - CHANGED TO BCE LOSS
    adv_loss = nn.BCEWithLogitsLoss()  # Changed from MSELoss to BCEWithLogitsLoss
    l1_loss = nn.L1Loss()
    ...
    for epoch in range(args.num_epochs):
        ...
        for sk, fc in loop:
            ...
            lossD_real = adv_loss(D_real, real_labels)
            
            fake = generator(sk)
            D_fake = discriminator(fake.detach())
            lossD_fake = adv_loss(D_fake, fake_labels)
            ...
            lossG_adv = adv_loss(D_fake2, real_labels)  # Generator wants discriminator to output 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sketch to Face GAN Training with BCE Loss')
